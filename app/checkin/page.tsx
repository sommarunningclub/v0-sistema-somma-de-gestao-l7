        ...
        <div className="sticky top-0 z-20 bg-neutral-950 border-b border-neutral-800/60">
            ...
            >
              ...
              {!mobileHeaderExpanded && (
                <div className="flex items-center gap-1.5 ml-1">
                  <span className="text-neutral-500 text-[10px] font-mono">{checkInData.length}</span>
                  {activeFilterCount > 0 && (
              ...
              <ChevronDown className={`w-3.5 h-3.5 text-neutral-500 transition-transform ${mobileHeaderExpanded ? 'rotate-180' : ''}`} />
            </button>
          ...
          {mobileHeaderExpanded && (
            <>
              ...
              {/* Filters */}
              <div className="px-4 py-2 border-t border-neutral-800/60 space-y-2.5">
                {/* Sort toggle */}
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <select
                          ...
                          className="w-full appearance-none bg-neutral-900 border border-neutral-800 text-white rounded-lg px-3 py-1.5 pr-8 text-[11px] font-medium outline-none focus:border-blue-500/50 transition-colors cursor-pointer capitalize"
                        >
                          <option value="">Todos ({checkInData.length})</option>
                          {uniqueSexos.map(s => (
                            <option key={s} value={s} className="capitalize">{s} ({statsBySexo(s)})</option>
                          ))}
                        </select>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <select
                          ...
                          className="w-full appearance-none bg-neutral-900 border border-neutral-800 text-white rounded-lg px-3 py-1.5 pr-8 text-[11px] font-medium outline-none focus:border-purple-500/50 transition-colors cursor-pointer"
                        >
                          <option value="">Todos ({checkInData.length})</option>
                          {uniquePelotoes.map(p => (
                            <option key={p} value={p}>{p} ({statsByPelotao(p)})</option>
                          ))}
                        </select>
                        <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-neutral-500 pointer-events-none" />
                      </div>
                  ...
                  <div className="pb-1">
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full appearance-none bg-neutral-900 border border-neutral-800 text-white rounded-lg px-3 py-1.5 pr-8 text-[11px] font-medium outline-none focus:border-cyan-500/50 transition-colors cursor-pointer"
                      >
                        <option value="">Todos os dias ({checkInData.length})</option>
                        {uniqueDias.map(d => (
                          <option key={d} value={d}>{d} ({statsByDia(d)})</option>
                        ))}
                      </select>
                      <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-neutral-500 pointer-events-none" />
                    </div>
            ...
            <div>
              ...
              <div className="relative">
                <select
                  value={selectedSexo || ''}
                  onChange={e => setSelectedSexo(e.target.value || null)}
                  className="w-full appearance-none bg-neutral-800 border border-neutral-700 text-white rounded-xl px-4 py-2.5 pr-10 text-xs font-medium outline-none focus:border-blue-500/50 transition-colors cursor-pointer capitalize"
                >
                  <option value="">Todos ({checkInData.length})</option>
                  {uniqueSexos.map(s => (
                    <option key={s} value={s} className="capitalize">{s} — {statsBySexo(s)} inscritos</option>
                  ))}
                </select>
            ...
            <div>
              ...
              <div className="relative">
                <select
                  value={selectedPelotao || ''}
                  onChange={e => setSelectedPelotao(e.target.value || null)}
                  className="w-full appearance-none bg-neutral-800 border border-neutral-700 text-white rounded-xl px-4 py-2.5 pr-10 text-xs font-medium outline-none focus:border-purple-500/50 transition-colors cursor-pointer"
                >
                  <option value="">Todos ({checkInData.length})</option>
                  {uniquePelotoes.map(p => (
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    value={selectedDia || ''}
                    onChange={e => setSelectedDia(e.target.value || null)}
                    className="w-full appearance-none bg-neutral-800 border border-neutral-700 text-white rounded-xl px-4 py-2.5 pr-10 text-xs font-medium outline-none focus:border-cyan-500/50 transition-colors cursor-pointer"
                  >
                    <option value="">Todos os dias ({checkInData.length})</option>
                    {uniqueDias.map(d => (
                ...
                <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-4">
                  ...
                  <div className="relative max-w-xs">
                    <select
                      value={selectedSexo || ''}
                      onChange={e => setSelectedSexo(e.target.value || null)}
                      className="w-full appearance-none bg-neutral-800 border border-neutral-700 text-white rounded-lg px-3 py-2 pr-9 text-xs font-medium outline-none focus:border-blue-500/50 transition-colors cursor-pointer capitalize"
                    >
                      <option value="">Todos ({checkInData.length})</option>
                      {uniqueSexos.map(sexo => (
                        <option key={sexo} value={sexo} className="capitalize">{sexo} — {statsBySexo(sexo)} inscritos</option>
                      ))}
                    </select>
                ...
                <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-4">
                  ...
                  <div className="relative max-w-xs">
                    <select
                      value={selectedPelotao || ''}
                      onChange={e => setSelectedPelotao(e.target.value || null)}
                      className="w-full appearance-none bg-neutral-800 border border-neutral-700 text-white rounded-lg px-3 py-2 pr-9 text-xs font-medium outline-none focus:border-purple-500/50 transition-colors cursor-pointer"
                    >
                      <option value="">Todos ({checkInData.length})</option>
                      {uniquePelotoes.map(p => (
                        <option key={p} value={p}>{p} — {statsByPelotao(p)} inscritos</option>
                ...
                <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-4 lg:col-span-2">
                  ...
                  <div className="relative w-full max-w-xs">
                    <select
                      value={selectedDia || ''}
                      onChange={e => setSelectedDia(e.target.value || null)}
                      className="w-full appearance-none bg-neutral-800 border border-neutral-700 text-white rounded-lg px-3 py-2 pr-9 text-xs font-medium outline-none focus:border-cyan-500/50 transition-colors cursor-pointer"
                    >
                      <option value="">Todos os dias ({checkInData.length})</option>
                      {uniqueDias.map(d => (
                        <option key={d} value={d}>{d} — {statsByDia(d)} inscritos</option>
