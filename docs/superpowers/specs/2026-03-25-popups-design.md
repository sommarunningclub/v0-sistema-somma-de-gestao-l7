...
Admin creates popup in gestão system
  → saves to Supabase (service role key)
...
| user_session_id | text | Random ID stored in sessionStorage |
...
- Max file size: 5MB
- Path convention: `{popup_id}/{filename}`

### RLS Policies

**popups:**
- Public SELECT: `is_active = true AND start_date <= now() AND (end_date IS NULL OR end_date >= now())`
- Admin SELECT/INSERT/UPDATE/DELETE: `auth.role() = 'service_role'`
...
2. **Imagem** — drag-and-drop upload zone; shows preview after upload; accepts jpg/png/webp/gif ≤5MB; uploads to Supabase Storage on file selection (not on save)
...
**Behavior:**
- Image uploads immediately on file selection → stores URL in state
- Save button disabled until required fields filled
- On save: POST (create) or PATCH (edit)
- On cancel/close: if new image was uploaded but not saved, delete from storage
...
|--------|------|------|-------------|
| GET | `/api/popups` | session | List all popups with click counts |
| POST | `/api/popups` | session | Create popup |
| PATCH | `/api/popups/[id]` | session | Update popup fields |
| DELETE | `/api/popups/[id]` | session | Delete popup + remove image from storage |
| GET | `/api/popups/[id]/stats` | session | Return metrics + daily click series |
| POST | `/api/popups/upload` | session | Upload image to Supabase Storage, return public URL |

Authentication: follows existing session pattern (`somma_session` in localStorage checked server-side via cookie or request header — same pattern as other modules).
...
2. Fetch active popups matching current page from Supabase REST API
3. Filter by frequency using `localStorage` (`popup_shown_{id}`) and `sessionStorage` (`popup_session_{id}`)
4. Queue valid popups
5. Show first popup in queue
6. On close: show next in queue (if any)
7. On image click: record click to `popup_clicks` + redirect to `redirect_link`
...
Generated once per session: `Math.random().toString(36).slice(2)`, stored in `sessionStorage` as `popup_session_id`. Sent with every click event.
...
lib/services/
  popups.ts                     # Supabase queries
sql/
  005-create-popups.sql         # Migration
```
...
app/
  layout.tsx                    # Add <Script src="/popup-sdk.js" />
```
...
- `components/protected-route.tsx` — add `popups` to PermissionsType
- `app/systems/page.tsx` — add "Pop-ups" checkbox in the admin user management UI
- `app/page.tsx` — add conditional render of Popups module and sidebar entry
...
CREATE TABLE popups (
  ...
  frequency    text NOT NULL DEFAULT 'uma_vez' CHECK (frequency IN ('uma_vez', 'sessao', 'sempre')),
...
-- 6. RLS: public read active popups
...
-- 7. RLS: service role full access on popups
CREATE POLICY "Service role full access on popups"
ON popups FOR ALL
USING (auth.role() = 'service_role')
WITH CHECK (auth.role() = 'service_role');

-- 8. RLS: public insert on popup_clicks
CREATE POLICY "Public can insert clicks"
ON popup_clicks FOR INSERT
WITH CHECK (true);

-- 9. RLS: service role read clicks
CREATE POLICY "Service role can read clicks"
ON popup_clicks FOR SELECT
USING (auth.role() = 'service_role');

-- 10. Storage bucket (run in Supabase dashboard > Storage > New bucket)
-- Name: popup-images
-- Public: true
-- OR via SQL:
...
-- 13. Storage RLS: service role delete
